...
import { FREE_PLAN_CANDIDATE_MESSAGE_LIMIT, getBillingPlan } from '~~/shared/billing'
  ...
  >
    <div class="flex items-start gap-3">
      ...
      <div class="min-w-0 flex-1">
        <p class="text-sm font-semibold text-surface-950 dark:text-white">Keep the conversation moving</p>
        <p class="mt-1 text-xs leading-5 text-surface-600 dark:text-surface-300">
          You've used all {{ FREE_PLAN_CANDIDATE_MESSAGE_LIMIT }} free candidate emails. Incoming replies stay visible. Upgrade to Solo to reply and continue every conversation.
        </p>

        <div v-if="!compact" class="mt-3 flex flex-wrap gap-x-4 gap-y-1 text-xs font-medium text-surface-700 dark:text-surface-200">
          <span class="inline-flex items-center gap-1.5"><Check class="size-3.5 text-success-600" /> Unlimited candidate messaging</span>
